...
async function requireAdmin() {
  ...
  const { data: profile } = await supabase.from("profiles").select("is_admin").eq("id", user.id).returns<Pick<ProfileRow, "is_admin">>().single();
  if (!profile?.is_admin) redirect("/login?error=Admin%20access%20required");
