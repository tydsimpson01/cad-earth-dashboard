...
export function PlayerCard({ player }: { player: Player }) {
  return <article className="player-card"><div className="player-head"><div><span className="player-role">{player.role}</span><div className="player-id">{player.id}</div></div><strong>{player.rank}</strong></div><div className="stat-strip"><div className="mini-stat"><strong>{player.winRate}</strong><span>WIN RATE</span></div><div className="mini-stat"><strong>{player.kda}</strong><span>KDA</span></div><div className="mini-stat"><strong>{player.csPerMinute}</strong><span>CS / MIN</span></div><div className="mini-stat"><strong>{player.killParticipation}</strong><span>AVG K+A / GAME</span></div></div><p><span className="eyebrow">CHAMPION POOL</span></p><div className="champions">{player.champions.map((champion) => <span className="champion-pill" key={champion}>{champion}</span>)}</div><div className="coach-callout"><span className="callout-icon">Ã¢Å“â€œ</span><p><b>Current goal:</b> {player.goal}</p></div></article>;
}
